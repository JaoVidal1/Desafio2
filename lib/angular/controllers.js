...
crud.controller('Desafio2Controller', function ($scope, $http) {
  ...
  $scope.editarJogador = function () {
    const jogadoresSelecionados = $scope.jogadores.filter(jogador => jogador.selecionado)
    jogadoresSelecionados.forEach(jogador => {

      var idUser = jogador._id;
      $http
        .put(
          'http://localhost:3000/jogadores/' + idUser , $scope.jogador
        )
        .then(() => {
          delete $scope.jogadores;
